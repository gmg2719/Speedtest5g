...
public class OnTransitionTextListener implements Indicator.OnTransitionListener {
	...
	private ColorGradient gradientBg;
	...
	public OnTransitionTextListener(float selectSize, float unSelectSize, int selectColor, int unSelectColor) {
		super();
		setColor(selectColor, unSelectColor,0);
	...
	public final OnTransitionTextListener setColorId(Context context, int selectColorId, int unSelectColorId) {
		...
		setColor(res.getColor(selectColorId), res.getColor(unSelectColorId),0);
		return this;
	}

	public final OnTransitionTextListener setColorBgId(Context context, int selectColorId, int unSelectColorId) {
		Resources res = context.getResources();
		setColor(res.getColor(selectColorId), res.getColor(unSelectColorId),1);
		return this;
	}
	...
	public final OnTransitionTextListener setColor(int selectColor, int unSelectColor,int type) {
		if (type == 0) {
			gradient = new ColorGradient(unSelectColor, selectColor, 100);
		}else if (type == 1) {
			gradientBg = new ColorGradient(unSelectColor, selectColor, 100);
		}
		return this;
	}
	...
	public void onTransition(View view, int position, float selectPercent) {
		...
		if (view instanceof TextView) {
			selectTextView = getTextView(view, position);
			updateTextView(selectTextView,selectPercent);
		}
		if (gradientBg != null) {
			view.setBackgroundColor(gradientBg.getColor((int) (selectPercent * 100)));
		}
	}

	private void updateTextView(TextView selectTextView,float selectPercent){
