...
import cn.nokia.speedtest5g.view.viewpager.Indicator.IndicatorAdapter;
import cn.nokia.speedtest5g.view.viewpager.Indicator.OnItemSelectedListener;
import cn.nokia.speedtest5g.view.viewpager.Indicator.OnTransitionListener;
...
public class IndicatorViewPager {
	...
	private OnItemSelectedListener onItemSelectedListener = new OnItemSelectedListener() {
		...
		public void onItemSelected(View selectItemView, int select, int preSelect) {
			if (isNullFragmentPage(select)) {
				indicatorView.setCurrentItem(preSelect, true);
				viewPager.setCurrentItem(preSelect, true);
				((IndicatorFragmentPagerAdapter) adapter).getListenerBack().onListener(select, preSelect, true);
				return;
			}
	...
	private ViewPager.OnPageChangeListener onPageChangeListener = new ViewPager.OnPageChangeListener() {
		...
		public void onPageSelected(int position) {
			if (isNullFragmentPage(position)) {
				indicatorView.setCurrentItem(indicatorView.getCurrentItem(), true);
				viewPager.setCurrentItem(indicatorView.getCurrentItem(), true);
				((IndicatorFragmentPagerAdapter) adapter).getListenerBack().onListener(position, indicatorView.getCurrentItem(), true);
				return;
			}
	...
	/**
	 * 判断是否为空页面，如果为空，可以设置监听在ListenBcck回调中处理
	 * 
	 * @param selectedPosition
	 * @return
	 */
	private boolean isNullFragmentPage(int selectedPosition) {
		boolean isNullFragmentPage = false;
		if (adapter instanceof IndicatorFragmentPagerAdapter) {
			IndicatorFragmentPagerAdapter ifpAdapter = (IndicatorFragmentPagerAdapter) adapter;
			Fragment fragment = ifpAdapter.getFragmentForPage(selectedPosition);
			if (fragment == null) {
				isNullFragmentPage = true;
			}
		}
		return isNullFragmentPage;
	}
	...
	public static abstract class IndicatorFragmentPagerAdapter implements IndicatorPagerAdapter {
		...
		public IndicatorFragmentPagerAdapter(FragmentManager fragmentManager, ListenerBack listenerBack) {
			super();
