...
//	private Drawable centerDrawable;
	...
	private void initData(Context context) {
		TEXT_SIZE = (int) getSPtoPx(context,TypedValue.COMPLEX_UNIT_SP, 12f);
	 ...
	 * @return
	 */
	private float getSPtoPx(Context context,int unit, float size) {
	       Resources r;
...
//		}
	...
	private int calculateLayoutWidth(int widthSize, int mode) {
		...
		if (itemsWidth > 0) {
			try {
				//这里处理因为长度过长而导致换行出现的问题，超出长度用...显示
				String[] strContents = ((StrericWheelAdapter)adapter).getStrContents();
				for (int i = 0; i < strContents.length; i++) {
					//总需要的宽度
					if (valuePaint.measureText(strContents[i]) >= itemsWidth) {
						//单个字符需要的宽度
						float aveTvWidth = valuePaint.measureText(strContents[i], 0, 1);
						strContents[i] = String.format("%s%s", strContents[i].subSequence(0, (int) (itemsWidth/aveTvWidth) - 4),"...");
					}
				}
				((StrericWheelAdapter)adapter).setStrContents(strContents);
...
//		centerDrawable.draw(canvas);
		...
		paint.setColor(Color.rgb(72, 89, 109));
		canvas.drawLine(getWidth()/10, center - offset, getWidth()-getWidth()/10,center - offset , paint);
		canvas.drawLine(getWidth()/10, center + offset, getWidth()-getWidth()/10, center + offset, paint);
	}
	...
	void finishScrolling() {
		if (mIRefresh!=null) {
			mIRefresh.onCall(getId(), getCurrentItemValue(),getCurrentItem());
		}
		if (isScrollingPerformed) {
	...
	//控件值(月份)改变时回调
	private OnWheelCallback mIRefresh;
		
	public void setListener(OnWheelCallback inter){
		this.mIRefresh = inter;
	}
